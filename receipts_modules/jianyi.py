    ...
    for i in range(len(pos)):
        if '收货人' in value[i]:
            for i in range(len(pos)):
                ...
                if 241<pos[i][1][0]-pos[i][0][0]<341 and 6<pos[i][2][1]-pos[i][0][1]<72 and 288<pos[i][0][1]<348:
                    return value[i]
                    #print('收货人:',value[i])
                    break
    ...
    for i in range(len(pos)):
        if '发货人' in value[i]:
            for i in range(len(pos)):
                ...
                if 214<pos[i][1][0]-pos[i][0][0]<414 and 7<pos[i][2][1]-pos[i][0][1]<50 and 350<pos[i][0][1]<500:
                    return value[i]
                    #print('发货人:',value[i])
                    break
    ...
    for i in range(len(pos)):
        if '品名' in value[i]:
            for i in range(len(pos)):
                ...
                if 27<pos[i][1][0]-pos[i][0][0]<227 and 2<pos[i][2][1]-pos[i][0][1]<80  and 440<pos[i][0][1]<500 and 258<pos[i][0][0]<318:
                    #print('品名:',value[i])
                    return value[i]
                    break
...
def match_zhongliang(pos,value):
    for i in range(len(pos)):
        if '报检数/重量' in value[i]:
            for i in range(len(pos)):
                if 65<pos[i][1][0]-pos[i][0][0]<265 and 2<pos[i][2][1]-pos[i][0][1]<180  and 442<pos[i][0][1]<502 and 864<pos[i][1][0]<1024:
    ...
    for i in range(len(pos)):
        if '输出国家或地区' in value[i]:
            if len(value[i])>7:
                ...
                for i in range(len(pos)):
                    if 30 < pos[i][1][0]-pos[i][0][0] < 100 and 5 < pos[i][2][1]-pos[i][0][1] < 50  and 540 < pos[i][0][1] < 580 and 700 < pos[i][0][0] < 780:
    ...
    for i in range(len(pos)):
        if '合同号' in value[i]:
            for i in range(len(pos)):
                ...
                if 10 < pos[i][1][0]-pos[i][0][0] < 176 and 5 < pos[i][2][1]-pos[i][0][1] < 127  and 603 < pos[i][0][1] < 663 and 259 < pos[i][0][0] < 319:
                    #print('合同号:',value[i])
                    return value[i]
    ...
    for i in range(len(pos)):
        ...
        if '提/运单号' in value[i]:
            for i in range(len(pos)):
                ...
                if 49 < pos[i][1][0]-pos[i][0][0] < 180 and 1 < pos[i][2][1]-pos[i][0][1] < 30  and 684 < pos[i][0][1] < 744 and 258 < pos[i][0][0] < 318:
                    #print('提/运单号:',value[i])
                    return value[i]
                    break
    ...
    for i in range(len(pos)):
        if '口岸' in value[i]:
            for i in range(len(pos)):
                ...
                if 9 < pos[i][1][0]-pos[i][0][0] < 144 and 5 < pos[i][2][1]-pos[i][0][1] < 126  and 763 < pos[i][0][1] < 823 and 259 < pos[i][0][0] < 319:
                    #print('入境口岸:',value[i])
                    return value[i]
                    break
...
def match_biaoji(pos,value):
    for i in range(len(pos)):
        if '标记及号码' in value[i]:
            for i in range(len(pos)):
                if 4 < pos[i][1][0]-pos[i][0][0] < 54 and 4 < pos[i][2][1]-pos[i][0][1] < 51  and 610 < pos[i][0][1] < 670 and 575 < pos[i][0][0] < 635:
                    #print('标记及号码:',value[i])
                    return value[i]
                    break
    ...
    for i in range(len(pos)):
        if '包装种类及数量' in value[i]:
            for i in range(len(pos)):
                ...
                if 10 < pos[i][1][0]-pos[i][0][0] < 307 and 5 < pos[i][2][1]-pos[i][0][1] < 124  and 523 < pos[i][0][1] < 583 and 259 < pos[i][0][0] < 319:
...
if __name__=='__main__':
    dict=OCR('pdf2img_14/images_11.png')
    print(dict[1])
    pos=dict[0]
    value=dict[1]
    ...
    def match_jianyi(pos,value):
        ...
        return {"编号":bianhao,"收货人":shouhuoren,"发货人":fahuoren,"品名":pinming,"报检数/重量":zhongliang,"输出国家":shuchuguojia,"集装箱号":jizhuangxiang,"生产日期":shengchanriqi,"生产厂家":shengchanchangjia,"品牌":pinpai,"规格":guige,"合同号":hetonghao,"运单号":yundanhao,"入境口岸":rujingkouan,"入境日期":rujingriqi,"标记":biaoji,"包装种类":baozhuangzhonglei,"备注":beizhu}
